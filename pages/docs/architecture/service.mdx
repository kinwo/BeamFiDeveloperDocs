...
[Beam](https://github.com/BeamFi/BeamFiProtocol/blob/main/backend/service/Beam.mo) is the central piece of BeamFi. It manages the creation of beam, streaming allocation update for each party, stopping, restarting, permission check and perisistence. There are 2 types of Beam:
...
[BeamEscrow](https://github.com/BeamFi/BeamFiProtocol/blob/main/backend/service/BeamEscrow.mo) is money keeper in BeamFi. It is where EscrowContract between sender and receipient is stored. It keeps track of the allocation of the token asset owned by each party. Each BeamType has 1:1 relationship with EscrowContract. It ensures the mathematics correctness of the contract everytime when Beam service updates its allocation. It is where self-custoday of the token asset is implemented as this actor will have the direct ownership in the token ledger (ICP / XTC). It is also where the token implementation is abstracted away from the Beam service. It is also responsible for the token transfer to the receipient when the token is claimed.
...
One of the most imporatant function is [privateUpdateEscrowAllocation](https://github.com/BeamFi/BeamFiProtocol/blob/23101f93860397a4360aac7d4a4ac2bcf6e8f32d/backend/service/BeamEscrow.mo#LL507C8-L507C37). It updates escrow contract allocation percentage for BeamEscrow actor, creator and buyer based on the request from Beam service actor. It also takes into account of overflow, invariants check and validation check.

## MonitorAgent

[MonitorAgent](https://github.com/BeamFi/BeamFiProtocol/blob/main/backend/service/MonitorAgent.mo) serves the purpose of monitoring the BeamFi protocol token solvency. At the moment, it supports ICP and XTC tokens. BTC token is in alpha stage. What it does is to verify all Contracts token for example ICP is smaller or equal to actual ICP tokens owned by BeamEscrow canister as reported by ICP ledger. It returns error if it fails. It expose HTTPS API /icp, /xtc for use by external monitoring service like UptimeRobot to regularly ping the HTTPS endpoint and notify admin support if it fails.
