...
The best way to experience with creating BeamEscrow is to use DFX command line. We can't use Candid UI because it doesn't support caller identity from web frontend.
...
`createBeamEscrow` is the top level service call to BeamFi. It will create a EscrowContract, BeamModel (by calling Beam Canister) and return the EscrowId. The EscrowId is the unique identifier for the EscrowContract.
...
For example, if you are using `dfx` command line, you can run following command to check your ICP balance:

```bash
dfx ledger balance --ledger-canister-id $(dfx canister id ledger) $(dfx ledger account-id)
```

Run following command to transfer 10 ICP tokens to the BeamEscrow canister:
...
`escrowAmount` is E8S format with base of 8 zeros meaning for ICP e.g 1 ICP = 100000000 E8S. For XTC, it is base of 12.
...
`creatorPrincipal` is the Principal ID of the recipient which you can get by creating a new identity running following in cmd line:
...
`dueDate` is the target complettion date of the payment streaming in epoch UTC time in nano seconds.

Run the following cmd to create a BeamEscrow EscrowContract with 10 ICP token, block index 1, due date 2023-05-27 15:49:15 UTC, buyerPrincipal is your Principal ID and creatorPrincipal is the recipient1 Principal ID.
