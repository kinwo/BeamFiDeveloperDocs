...
After creating BeamEscrow EscrowContract, either the sender or the receipient can lookup the BeamEscrowContracts owned by them.

```js
public query ({ caller }) func queryMyBeams() : async [BeamEscrowContract]
```

Run the following in command line as sender identity princiapl:
...
<Callout type="warning" emoji="⚠️">
  Note that althought this method is public, it will only return
  BeamEscrowContracts owned by the caller not others.
</Callout>

If you switch to receipient identity and run the same command, it should return the same BeamEscrowContracts.
