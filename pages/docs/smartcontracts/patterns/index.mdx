...
Also, Canister follows [Actor Model] (https://internetcomputer.org/capabilities/actor-model) which means its state can only be updated by itself through singel therad non-lock based execution through [consensus of blockchain which is about 2 secs](https://internetcomputer.org/icig.pdf). Only one update message can be processed at one time.
