...
This `Getting Started` guide demonstrates how to use BeamFi with ICP token.

### Setup Local Canister

This is special step to lock local ICP ledger canister ID to ryjl3-tyaaa-aaaaa-aaaba-cai for working with [Plug Wallet](https://plugwallet.ooo/) Local network `window.ic.plug.requestTransfer()` method.
