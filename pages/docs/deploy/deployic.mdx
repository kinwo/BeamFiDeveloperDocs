...
BeamFi is an open source real-time money streaming protocol. While you can use the current one deployed to public IC mainnet directly in your project, you can also deploy your own BeamFi Protocol smart contracts to IC mainnet and customize / enhance it for your own purpose.

Each instance of BeamFi smart contracts is called a BeamFi Vault. You can deploy as many BeamFi Vaults as you want to IC mainnet. Each BeamFi Vault is independent from each other and has its own set of smart contracts and properties.
...
Soon, BeamFi app would be enhanced to make connections to different BeamFi Vaults and allow users to switch between them.
...
If you come from Ethereum background, you can think of cycles as gas and ICP tokens as ETH.

[Learn more about ICP and Cycles](https://internetcomputer.org/docs/current/developer-docs/setup/cycles/)

Dfinity provides free Cycles for new developers to get started. You can request for free cycles from [IC Developer Portal](https://internetcomputer.org/docs/current/developer-docs/setup/cycles/cycles-faucet).
...
Export each of the key below as environment variable.

E.g. to export `CLIENT_KEY` as environment variable, run the following command.
...
| :----------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| CLIENT_KEY               | HTTPS API Request Client Key. It is used by all http_request http_request_update methods to verify the http client as basic protection. Simpley generate a secure random string about 26 chars using Password Manager |
| BEAM_ESCROW_CANISTER_ID  |                                                                                   BeamEscrow Canister ID. Get it from previous step                                                                                   |
| BEAM_CANISTER_ID         |                                                                                      Beam Canister ID. Get it from previous step                                                                                      |
| MONITORAGENT_CANISTER_ID |                                                                                  MonitorAgent Canister ID. Get it from previous step                                                                                  |
| BITCOIN_NETWORK          |                                                                             Bitcoin Network to connect to. Leave it to `Regtest` for now                                                                              |
| ZOOM_SECRET_TOKEN        |                                                                          Zoom Developer Account API Secret Token. Leave it to empty for now                                                                           |

Having configued the environment variables, you can now run the script to update Env.mo.
...
If you would like to deploy individual canister, you can run the following command.
