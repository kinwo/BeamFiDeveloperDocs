...
It is recommended to use a CI/CD pipeline to build and deploy your own BeamFi Vault. The following steps describe how to setup a pipeline using [Github Action](https://github.com/BeamFi/BeamFiProtocol/actions).

## Setup Github Action

BeamFi Protocol Git repository comes with Github Workflow yml files which you can use to setup your CI/CD pipeline.

For example, to deploy to production IC mainnet.

Use [.github/workflows/prod.yml](https://github.com/BeamFi/BeamFiProtocol/blob/main/.github/workflows/prod.yml)

It has simple workflow with two jobs, one for testing (by running 2 automated test scripts) and one for deployment.
...
<Callout type="warning" emoji="⚠️">
  Note that this workflow assumes you have follow [`Deploy your own BeamFi
...
| ./scripts/add-ic-identity.sh        | Create DFX identity and walletsfor production deployment |
...
Before running the workflow, you need to setup Github Secrets for the following environment variables.
...
**Retreive your DFX identity for DFX_IDENTITY_PROD**
...
Put the output in Github Actions secretst DFX_IDENTITY_PROD.
...
Get the output and replace identity name to "default".
