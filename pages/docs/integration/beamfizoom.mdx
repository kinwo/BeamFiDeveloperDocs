...
`http_request_update` func allows Beam canister to receive Zoom Webhook event when a meeting is started. The event are then processed by Beam canister which would update the state of the beam and BeamEscrowContract in BeamEscrow canister.

If you are interested setting up your own BeamFi integration with Zoom locally, it will require a Zoom developer account and a network tunnel for ingress-as-a-service e.g [Ngrok](https://ngrok.com/) to expose your localhost securely as Webhook URL in Zoom Webhook configs. The scope of it is beyond this documentation. Feel free to [reach out to us](mailto:support@beamfi.app) if you need help with it.
